...
type
  ...
  TSByteReceived = procedure(Sender: TObject; const Data: Int8) of object;
  ...
  TArrayReceived = procedure(Sender: TObject;
    const Data: TBytesArray) of object;
  ...
  private
    ...
    procedure DoSByteReceived(const Data: Int8);
    ...
    procedure DoArrayReceived(const Data: TBytesArray);
    ...
    procedure DecodeSendCommand(const Data: TBytesArray);
    procedure DecodeGetCommand(const Data: TBytesArray);
    procedure DecodeErrorCommand(const Data: TBytesArray);
    procedure DecodeData(const Data: TBytesArray);
  ...
  public
    ...
    procedure ProcessData(const Data: TBytesArray);
...
procedure TCommandDecoder.DecodeData(const Data: TBytesArray);
var
  Cmd: Byte;
begin
...
procedure TCommandDecoder.DecodeErrorCommand(const Data: TBytesArray);
var
  Error: Integer;
begin
...
procedure TCommandDecoder.DecodeSendCommand(const Data: TBytesArray);
var
  Arr: TBytesArray;
  Utf8: RawByteString;
  Str: string;
  ...
  case Data[0] and TCommands.CMD_FLAG of
    ...
    TCommands.CMD_STRING:
      if Length(Data) > 1 then begin
        ...
        Str := UTF8ToWideString(Utf8);
        DoStringReceived(Str);
      end;
  end;
end;

procedure TCommandDecoder.DoArrayReceived(const Data: TBytesArray);
begin
...
procedure TCommandDecoder.DoError(const Error: Integer);
begin
...
procedure TCommandDecoder.DoSByteReceived(const Data: Int8);
begin
...
procedure TCommandDecoder.ProcessData(const Data: TBytesArray);
var
  Ndx: Int32;
begin
